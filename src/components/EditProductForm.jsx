...
export default function EditProductForm({
  ...
  const [newStrengh, setNewStrengh] = useState(strengh);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch(`http://localhost:3000/api/products/${id}`, {
        ...
        body: JSON.stringify({ newName, newProject, newImplementation, newEmail, newMobile, newBudget, newYear, newEvaluation, newWeak, newStrengh, newDevelopment, newSuggestion }),
      });
  ...
  return (
    <>
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-3">
        ...
        />
        <input
          onChange={(e) => setNewStrengh(e.target.value)}
          value={newStrengh}
          className="input input-bordered input-accent w-full max-w-xs"
          type="text"
        />
